...
class CoffeeMachine {
  static coffeeFormulas = {
    "espresso": {"water": 250, "milk": 0, "coffeeBeans": 16, "dirtiness": 1},
    "latte": {"water": 350, "milk": 75, "coffeeBeans": 20, "dirtiness": 3},
    "cappuccino": {"water": 200, "milk": 100, "coffeeBeans": 12, "dirtiness": 4}
  };
...
$${this.money} of money`);
  ...
  buy() {
    ...
    if (this.dirtiness >= 10) {
      console.log("Coffee machine too dirty!");
      return;
    }
    ...
    if (check === 1) {
      ...
      let dirty = CoffeeMachine.coffeeFormulas[coffee]["dirtiness"];
      this.money += Number(price);
      this.water -= Number(water);
      this.milk -= Number(milk);
      this.coffeeBeans -= Number(coffeeBeans);
      this.disposable -= 1;
      this.dirtiness += dirty;
    } else {
      console.log(check)
    }
  }
...
while (true) {
      answer = input("Write action (buy, fill, take, remaining, clean, exit): ")
      let quit = false;
      switch(answer) {
        case "buy":
         machine.buy();
